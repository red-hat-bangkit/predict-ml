# -*- coding: utf-8 -*-
"""PREDICT (PRE DIsaster Consciousness Tips).ipynb

Automatically generated by Colaboratory.

Original file is located at
    https://colab.research.google.com/drive/1XKpJ-7igmJKR8gwwy59EsiHF0QjzQewC
...
# Asumsi Labeling (Due to incomplete data from PUPR)
...
# save preprocessed data (using pickle because with pandas.to_csv can't recover scaled information)
pickle.dump(df_tmrain_tmwater_days, open("df_tmrain_tmwater_days.pkl", "wb"))
# Save banjir mapping
...
df_tmrain_tmwater_days = pickle.load(open(os.path.join(BASE_DIR, "df_tmrain_tmwater_days.pkl"), "rb"))
def get_rainfall_by_location(location, upto_date=None):
  if upto_date:
    return df_tmrain_tmwater_days[["Date", "Location", "Rainfall", "Banjir"]][df_tmrain_tmwater_days["Location"] == location].sort_values("Date")[df_tmrain_tmwater_days["Date"] <= upto_date]
  return df_tmrain_tmwater_days[["Date", "Location", "Rainfall", "Banjir"]][df_tmrain_tmwater_days["Location"] == location].sort_values("Date")
...
# Bendungan Gintung before 2020-10-20

location_transform = json.load(open(banjir_mapping_location, "r"))["Location"]
def inverse_transform_rainfall(df):
    df["Rainfall"] = scaler.inverse_transform(df["Rainfall"].values.reshape(-1,1)).ravel()
    return df
inverse_transform_rainfall(
    df_tmrain_tmwater_days[df_tmrain_tmwater_days["Location"] == location_transform["Bendungan Gintung"]].sort_values("Date")[df_tmrain_tmwater_days["Date"] <= "2020-10-20"].tail(10)
)

from datetime import datetime
output, out_name = predict_banjir("Bendungan Gintung", future_days=10, after_date="2020-10-10")
...
## classification model
models = {}
kerasbinary_banjir = tf.keras.models.load_model("models/kerasbinary-predict-banjir.h5")
scaler = pickle.load(open(os.path.join(BASE_DIR, "scaler.pkl"), "rb"))
...
for location in locations:
  try:
    m = tf.keras.models.load_model("models/lstm-predict-banjir-%s.h5"%location)
    models[location] = m
  except Exception as err:
    print(err)
...
def predict_banjir(location: str, future_days=1, after_date=None):
  ...
  # predict banjir or no using tf model
  input_value = []
  for i in range(future_days):
    input_value.append([location, forecast_set[i]])
  # if ENABLE_OUTPUT:  print("input_value (scaled)=",input_value)
  ...
  for i in range(future_days):
    ...
    if predictions[i] > 0.5:
        ...
        prediction = 0
        confidence = 0.5 + 0.5 - predictions[i]
    output.append([prediction, rmses[i], forecast_set[i], pred_time, confidence])
  return output, ("prediction", "rmse", "forecasted_rainfall", "date", "confidence")
...
Because of that we decided to use the classification determining banjir or not banjir using tensorflow, whereas to predict regression of rain using simple linear regression.
"""
...
## classification model
models = {}
kerasbinary_banjir = tf.keras.models.load_model(os.path.join(BASE_DIR, "models/kerasbinary-predict-banjir.h5"))
scaler = pickle.load(open(os.path.join(BASE_DIR, "scaler.pkl"), "rb"))
models["kerasbinary-banjir"] = kerasbinary_banjir
